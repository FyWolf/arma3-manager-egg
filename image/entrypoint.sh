...
## A3M === ARMA 3 MANAGER ===
# Constants and the status.json functions live in a3m-common.sh, because
# the background sync daemon (a3m-sync.sh) reports through the same file
# and two copies of that logic would drift invisibly.
source /a3m-common.sh
# Color Codes
CYAN='\033[0;36m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
...
## A3M === BACKGROUND SYNC ===
#
# Start the mod downloader alongside the game, so the panel can fetch mods at any
# time without stopping or restarting anything.
#
# It starts *here*, after the boot-time update has finished, and that ordering is
# the whole safety argument: two SteamCMD processes against one Steam account and
# one workshop directory is a race over the same files, and the second one loses
# in ways that look like a corrupt mod. By this line the boot update is done.
#
# It downloads into the SteamCMD cache and does not build `@<id>` folders — those
# are what this running server has open. See a3m-sync.sh.
if [[ ${A3M_BACKGROUND_SYNC} != "0" ]] && [[ ${A3M_DISABLE} != "1" ]] && [[ -x /a3m-sync.sh ]]; then
    /a3m-sync.sh daemon &
    A3M_SYNC_PID=$!

    # Kill it with us. Without this the daemon survives as an orphan when the
    # server stops, and Wings' next start would have two of them polling the
    # same request file.
    trap 'kill "${A3M_SYNC_PID}" 2>/dev/null' EXIT INT TERM

    echo -e "\n${GREEN}[A3M]:${NC} Background mod sync started. Mods can be downloaded while the server runs."
fi
